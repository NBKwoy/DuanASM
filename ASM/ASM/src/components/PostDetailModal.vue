...
const sendComment = () => {
    ...
    // ... (Logic gửi bình luận giống bài trước) ...
    if (replyingTo.value) {
        const postInStore = store.posts.find(p => p.id === props.post.id);
        const parentComment = postInStore.comments.find(c => c.id === replyingTo.value.parentId);
        if (parentComment) {
            if (!parentComment.replies) parentComment.replies = [];
            parentComment.replies.push({
                id: Date.now(), userId: currentUser.id, content: newComment.value, timestamp: new Date().toISOString(), replyToUser: replyingTo.value.targetName, likes: []
            });
            if (replyingTo.value.targetId !== currentUser.id) sendNotification(replyingTo.value.targetId, 'reply', `đã trả lời bình luận của bạn.`);
        }
    } else {
        const postInStore = store.posts.find(p => p.id === props.post.id);
        if (!postInStore.comments) postInStore.comments = [];
        postInStore.comments.push({
            id: Date.now(), userId: currentUser.id, content: newComment.value, timestamp: new Date().toISOString(), replies: [], likes: []
        });
        if (props.post.userId !== currentUser.id) sendNotification(props.post.userId, 'comment', `đã bình luận bài viết của bạn.`);
    }
    newComment.value = ''; replyingTo.value = null; store.saveDB();
};
                    ...
                    <div class="col-lg-4 col-md-5 bg-white d-flex flex-column h-100 position-relative">
                        ...
                        <div class="p-3 border-top bg-white">
                            ...
                            <div v-if="!replyingTo" class="d-flex justify-content-between mb-2 px-1">
                                <button @click="toggleLike" class="btn btn-link text-decoration-none p-0 fw-bold small" :class="post.likes?.includes(store.currentUser.id) ? 'text-danger' : 'text-secondary'">
                                    <i :class="post.likes?.includes(store.currentUser.id) ? 'fas' : 'far'" class="fa-heart me-1"></i> {{ post.likes?.length || 0 }}
                                </button>

...
import CreatePostModal from './CreatePostModal.vue'; // Import mới
import PostDetailModal from './PostDetailModal.vue'; // Import mới

// ... (Giữ nguyên các biến filterMode, categories, isYoutube, getYoutubeEmbed, handleTrendSearch...)
// ... (Hãy copy lại các hàm logic từ file Home.vue cũ sang đây, chỉ thêm phần dưới này)

const emit = defineEmits(['navigate']);

// --- STATE CHO MODAL ---
const showCreateModal = ref(false);
const selectedPost = ref(null); // Bài viết đang xem chi tiết
...
// ... (Logic filteredPosts, createPost, toggleLike GIỮ NGUYÊN) ...
// ...
// (Để ngắn gọn, tôi chỉ paste phần thay đổi chính trong <template>, bạn giữ script cũ và thêm các import/state trên)

// --- CÁC LOGIC CŨ CẦN GIỮ LẠI (Copy paste vào đây) ---
// Danh mục Menu Trái
const categories = [
...
const handleTrendSearch = (keyword) => { filterMode.value = 'trend'; currentTrendKeyword.value = keyword; setTimeout(() => { const feed = document.getElementById('main-feed'); if(feed) feed.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100); };
        ...
        <div class="hero-header position-relative d-flex align-items-center justify-content-center flex-column text-white">
            ...
            <div class="search-container z-2 w-100 px-3" style="max-width: 700px;">
                <div class="input-group input-group-lg bg-white rounded-pill overflow-hidden shadow-lg border border-3 border-warning">
                    ...
                    <input v-model="bigSearchQuery" @keyup.enter="handleBigSearch" class="form-control border-0 shadow-none py-3" placeholder="Tìm kiếm...">
        ...
        <div class="container py-4">
            <div class="row g-4">
                    ...
                    <div class="sticky-sidebar">
                        <div class="list-group shadow-sm rounded-4 overflow-hidden border-0 bg-white">
                            <button v-for="cat in categories" :key="cat.id" @click="selectCategory(cat.id)"
                                class="list-group-item list-group-item-action border-0 py-3 fw-bold d-flex align-items-center"
                                :class="(filterMode === 'category' && activeCategory === cat.id) ? 'bg-orange-light text-orange border-start border-4 border-orange' : 'text-secondary'">
                                <i :class="cat.icon" class="me-3 fs-5" style="width: 25px;"></i> {{ cat.name }}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-7 col-12" id="main-feed">
                    <div class="d-lg-none mb-3 overflow-auto d-flex pb-2 hide-scrollbar">
                        <button v-for="cat in categories" :key="cat.id" @click="selectCategory(cat.id)"
                            class="btn btn-sm rounded-pill me-2 text-nowrap fw-bold px-3"
                            :class="(filterMode === 'category' && activeCategory === cat.id) ? 'btn-orange text-white' : 'btn-light border'">{{ cat.name }}</button>
                    </div>
                    ...
                    <div class="card shadow border-0 mb-4 rounded-4">
                        <div class="card-body p-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div @click="showCreateModal = true" class="bg-light rounded-pill px-3 py-2 flex-grow-1 text-muted cursor-pointer hover-bg-darker transition-btn">
                                    Bạn đang nghĩ gì thế, {{ store.currentUser.name }}?
                                </div>
                            </div>
                    ...
                    <div v-for="post in filteredPosts" :key="post.id" class="card shadow-sm border-0 mb-4 rounded-4 hover-shadow animate-up">
                        <div class="card-body p-4">
                            ...
                            <div v-if="post.image" @click="openPostDetail(post)" class="rounded-3 overflow-hidden mb-3 bg-black d-flex justify-content-center align-items-center cursor-pointer position-relative media-preview" style="max-height: 400px;">
                                ...
                                <div v-if="isYoutube(post.image)" class="ratio ratio-16x9 w-100 pointer-events-none">
                                    <iframe :src="getYoutubeEmbed(post.image)" title="YouTube" class="pointer-events-none"></iframe>
                                </div>
                            ...
                            <div class="d-flex justify-content-between text-muted border-top pt-3">
                                <button @click.stop="toggleLike(post)" class="btn flex-grow-1 border-0 fw-bold transition-btn" :class="post.likes?.includes(store.currentUser.id) ? 'text-orange' : 'text-muted'">
                                    <i :class="post.likes?.includes(store.currentUser.id) ? 'fas' : 'far'" class="fa-thumbs-up me-2"></i>{{ post.likes?.length || 0 }}
                                </button>
                                <button @click="openPostDetail(post)" class="btn btn-light flex-grow-1 border-0 text-muted transition-btn">
                                    <i class="far fa-comment-dots me-2"></i>Bình luận
                                </button>
                                <button class="btn btn-light flex-grow-1 border-0 text-muted transition-btn">
                                    <i class="fas fa-share me-2"></i>{{ post.shares || 0 }}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-3 d-none d-lg-block">
                    <div class="sticky-sidebar">
                        <HotSearch @search-trend="handleTrendSearch" />
                    </div>
                </div>
            </div>
        </div>
...
<style scoped>
/* CSS cũ giữ nguyên, thêm 1 chút cho UI đẹp hơn */
...
.bg-orange-light { background-color: #fff0e0 !important; }
.border-orange { border-color: #ff8200 !important; }
.sticky-sidebar { position: -webkit-sticky; position: sticky; top: 90px; z-index: 100; }
.page-background { background-color: #f0f2f5; min-height: 100vh; padding-top: 0; }
.hover-bg-darker:hover { background-color: #e9ecef !important; }
.pointer-events-none { pointer-events: none; } /* Quan trọng để click xuyên qua iframe/video khi ở feed */
.media-preview:hover .overlay-hover { opacity: 1; }
